...
use App\Models\CampaignDonationAdjustment;
...
class CampaignDonationController extends Controller
{
    ...
    private function assertCanManageDonation(?User $user): void
    {
        ...
        if ($user->can('finance.view') || $user->can('campaigns.manage')) {
            return;
        }
        abort(403);
    }

    public function storeManual(Request $request, DonationCampaign $donationCampaign): RedirectResponse
    {
        $this->assertCanManageDonation($request->user());

        if ($donationCampaign->status === DonationCampaign::STATUS_ARCHIVED) {
        ...
        if ($memberId !== null) {
            ...
            if (! $member || $member->church_id !== $donationCampaign->church_id) {
                return redirect()->back()->withErrors([
        ...
        if ($request->hasFile('receipt')) {
            ...
            if (CampaignDonation::query()->where('receipt_hash', $receiptHash)->exists()) {
                return redirect()->back()->with('error', 'Este comprovante já foi utilizado em outra doação.');
            }
            $receiptPath = $file->store('donations/receipts', 'public');
        }

        $amount = round((float) $data['amount'], 2);
        $confirmedAt = isset($data['confirmed_at']) ? \Carbon\Carbon::parse($data['confirmed_at']) : now();

        $donation = DB::transaction(function () use ($request, $donationCampaign, $data, $memberId, $externalName, $receiptPath, $receiptHash, $amount, $confirmedAt) {
    ...
    public function updateAmount(Request $request, CampaignDonation $campaignDonation): RedirectResponse
    {
        $this->assertCanManageDonation($request->user());
        ...
        $oldAmount = (float) $campaignDonation->amount;

        if ($newAmount === $oldAmount) {
            return redirect()->back()->with('error', 'Informe um valor diferente do atual para ajustar.');
        }

        DB::transaction(function () use ($request, $campaignDonation, $data, $newAmount, $oldAmount) {
            $donation = CampaignDonation::query()->lockForUpdate()->findOrFail($campaignDonation->id);
            $campaign = DonationCampaign::query()->lockForUpdate()->findOrFail($donation->campaign_id);
            ...
            CampaignDonationAdjustment::create([
    ...
    public function resolveDispute(Request $request, CampaignDonation $campaignDonation): RedirectResponse
    {
        $this->assertCanManageDonation($request->user());

        if ($campaignDonation->dispute_status !== self::DISPUTE_PENDING) {
