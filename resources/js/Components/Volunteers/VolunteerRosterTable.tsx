...
const stickyShadowRight =
    'shadow-[6px_0_12px_-6px_rgba(0,0,0,0.12)] dark:shadow-[6px_0_12px_-6px_rgba(0,0,0,0.45)]';
const stickyShadowLeft =
    'shadow-[-6px_0_12px_-6px_rgba(0,0,0,0.12)] dark:shadow-[-6px_0_12px_-6px_rgba(0,0,0,0.45)]';
const stickyHeadBg = 'bg-zinc-50 dark:bg-zinc-950';

function stickyBodyBg(pendingInvite: boolean): string {
    if (pendingInvite) {
        return 'bg-amber-50 group-hover:bg-amber-100/90 dark:bg-amber-950 dark:group-hover:bg-amber-950';
    }

    return 'bg-white group-hover:bg-zinc-50 dark:bg-zinc-900 dark:group-hover:bg-zinc-800';
}
...
export default function VolunteerRosterTable({
    ...
    return (
        <div className="overflow-x-auto">
            <table className={`min-w-[720px] w-full text-left ${textSize}`}>
                <thead className={`sticky top-0 z-10 ${stickyHeadBg} ${headClass}`}>
                    <tr className="border-b border-zinc-200 dark:border-zinc-700">
                        <th
                            className={`sticky left-0 z-30 w-10 min-w-10 pb-2 pr-2 font-semibold ${stickyHeadBg}`}
                        >
                            <span className="sr-only">Foto</span>
                        </th>
                        <th
                            className={`sticky left-10 z-30 min-w-[7.5rem] max-w-[10rem] pb-2 pr-3 font-semibold ${stickyHeadBg} ${stickyShadowRight}`}
                        >
                            Nome
                        </th>
                        ...
                        <th
                            className={`sticky right-0 z-30 pb-2 pl-2 text-right font-semibold ${stickyHeadBg} ${stickyShadowLeft}`}
                        >
                            Ações
                        </th>
                    </tr>
                ...
                <tbody>
                    {volunteers.map((v) => {
                        const bodyStickyBg = stickyBodyBg(Boolean(v.pendingInvite));
                        ...
                        >
                            <td className={`sticky left-0 z-20 w-10 min-w-10 ${cellY} pr-2 ${bodyStickyBg}`}>
                                <UserListAvatar name={v.name} photoUrl={v.photoUrl} size="sm" previewOnClick={false} />
                            </td>
                            <td
                                className={`sticky left-10 z-20 min-w-[7.5rem] max-w-[10rem] ${cellY} pr-3 font-medium text-zinc-900 dark:text-white ${bodyStickyBg} ${stickyShadowRight}`}
                            >
                                <div className="flex flex-wrap items-center gap-1.5">
                                    ...
                                    <span className="truncate" title={v.name ?? undefined}>
                                        {v.name}
                                    </span>
                                    {v.hasUserAccount ? (
                            ...
                            <td
                                className={`sticky right-0 z-20 cursor-default ${cellY} pl-2 text-right ${bodyStickyBg} ${stickyShadowLeft}`}
