...
export default function RevistaAdventistaIndex({ editions, canManage, availableYears, filters }: Props) {
    const [syncing, setSyncing] = useState(false);
    const [syncingPdfs, setSyncingPdfs] = useState(false);

    const applyYear = (year: number) => {
        router.get(route('revista-adventista.index'), buildQuery({ ...filters, ano: year }), {
            preserveState: true,
            preserveScroll: true,
            replace: true,
        });
    ...
    const applyStatus = (status: 'all' | 'active' | 'inactive') => {
        router.get(route('revista-adventista.index'), buildQuery({ ...filters, status }), {
            preserveState: true,
            preserveScroll: true,
            replace: true,
        });
    ...
    const toggleActive = async (edition: Edition, isActive: boolean) => {
        ...
        router.patch(route('revista-adventista.edition.active', edition.id), { is_active: isActive }, { preserveScroll: true });
    };
    ...
    const syncArchive = (cachePdfs: boolean) => {
        ...
        router.post(
            route('revista-adventista.sync-archive'),
